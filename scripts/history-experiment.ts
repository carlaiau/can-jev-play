...
const noul=process.argv.includes('--noul');
...
const base=process.argv.find(a=>a.startsWith('--resume='))?.slice(9)??`reports/${noul?'noul-window':'history-window'}-${Date.now()}`;
if(!/^reports\/(?:history|noul)-window-\d+$/.test(base))throw new Error('Invalid run path');
await mkdir('reports',{recursive:true});
const lock=await open(`${base}.lock`,'wx');
try {
 if(noul && (process.argv.includes('--no-history') || process.argv.some(a=>a.startsWith('--rolls=')) ))throw Error('Noul uses the complete matched source dataset; omit --rolls and --no-history');
 const source=noul?JSON.parse(await readFile('src/data/history-study.json','utf8')) as HistoryStudy:undefined;
 ...
 const plan=noul?source!.rows.map(({id,magnitude,number,scenario,ev,gross,history,state})=>({id,magnitude,number,scenario,ev,gross,history,state})):noHistory?prior!.rows.filter(r=>r.scenario==='mixed').map(r=>{const {recentRolls,...state}=r.state;return {id:`no-history-${r.magnitude}-${r.number}`,magnitude:r.magnitude,number:r.number,scenario:'no-history' as const,ev:r.ev,gross:r.gross,history:[],state};}):interleaved?interleavedPlan(count):historyPlan(count);
 const requestedArms:Arm[]=noul||noHistory||interleaved?['table']:['table','calculated'];
 const requestFor=(trial:typeof plan[number],arm:Arm)=>({...historyRequest(trial,arm),...(noul?{questions:noulQuestions}:{})});
 const manifest={plan,questions:requestFor(plan[0],'table').questions,...(noul?{source:source!.provenance,model:'jev-1.13.0',decisionRule:'Bet iff noul > 0.5; otherwise Skip'}:{})};
 ...
 async function worker(){while(!failed){const job=jobs[next++];if(!job)return;const {trial,arm}=job;const request=requestFor(trial,arm);
  try {let decision:ExperimentDecision|undefined;
   for(let attempt=0;attempt<8;attempt++){
    ...
    try{
     ...
     const parsed=(noul?parseNoulAction:parseBinaryAction)(JSON.parse(raw));if(parsed.response.model!==request.model)throw Error('Fatal model mismatch');
 ...
 await writeFile(`${base}.json`,JSON.stringify(study));const destination=noul?'src/data/noul-study.json':'src/data/history-study.json';await writeFile(`${destination}.tmp`,JSON.stringify(study));await rename(`${destination}.tmp`,destination);
